...
The repo now includes platform-only internal workspace/app-access seed contracts. The seed service can idempotently prepare an internal workspace, the `sqag` app registry record, the workspace entitlement, and an admin/operator/viewer membership grant through storage-agnostic repository ports. It can grant access to an existing authenticated user by user id or normalized email. Creating a user and provider identity together is deferred until an explicit transactional identity seed boundary exists, so the seed refuses provider-identity user creation before any platform writes. It also refuses email-only user precreation for future provider linking, fake-login shortcuts, conflicting records, and viewer grants for SQAG.
