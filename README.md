...
The repo now includes platform-only internal workspace/app-access seed contracts. The seed service can idempotently prepare an internal workspace, the `sqag` app registry record, the workspace entitlement, and an owner/admin/member membership grant through storage-agnostic repository ports. It can grant access to an existing authenticated user by user id or normalized email. Creating a user and provider identity together is deferred until an explicit transactional identity seed boundary exists, so the seed refuses provider-identity user creation before any platform writes. It also refuses email-only user precreation for future provider linking, fake-login shortcuts, conflicting records, and viewer grants for SQAG.
...
The repo now includes an explicit internal platform access seed CLI. In first-owner bootstrap mode, `npm run platform:seed-internal-access` with `PLATFORM_SEED_BOOTSTRAP_MODE=first-owner-pending-approval`, `PLATFORM_SEED_CONFIRM=seed-reviewed-internal-access`, `PLATFORM_SEED_USER_EMAIL`, `PLATFORM_SEED_WORKSPACE_SLUG`, and `PLATFORM_SEED_WORKSPACE_NAME` creates or reuses only the reviewed workspace, `sqag` app record, workspace entitlement, and a pending owner approval for the reviewed email. The first real OIDC sign-in for that email then creates the provider-backed Platform user/provider identity/session and transactionally activates the owner membership. For an already-authenticated provider-backed user, the same CLI without `PLATFORM_SEED_BOOTSTRAP_MODE` can still seed owner/admin/member access. The CLI validates confirmation, email, and explicit workspace identity before opening a DB connection, prints only a safe summary, and does not run migrations. It does not create fake provider identities, sessions, app launch tokens, fake login state, SQAG storage, default production workspace data, private SQAG profile/pricing data, or deployment artifacts.

Safe internal setup flow:

1. Configure real OIDC and platform runtime env.
2. Build and run `npm run platform:start`.
3. Run `npm run platform:seed-internal-access` in first-owner bootstrap mode with the required reviewed seed env.
4. The reviewed first owner signs in through `/api/platform/auth/start` with real OIDC.
5. Visit `/app`.
