...
The internal smoke checklist is documented in `docs/internal-platform-smoke-runbook.md`. It covers the existing database/OIDC/runtime/seed/browser-launch path, keeps migrations explicit, and documents the local same-host SQAG handoff shape without putting launch tokens in browser URLs or storage.

The hosted internal-alpha runbook is documented in `docs/hosted-internal-alpha-runbook.md`. It covers hosted Platform/SQAG placeholders, the Neon Postgres readiness target, the env checklist, manual migration/backup/rollback procedure, smoke checklist, dry-run env readiness, and one-off DB readiness check without deploying, provisioning, or exposing infrastructure. The env readiness check is hardened for hosted review: it requires production mode, HTTPS browser/provider-facing URLs, origin-only allowed origins, callback path shape, SQAG handoff URL guardrails, valid Postgres-shaped `DATABASE_URL`, value-safe output, and no migration/server/network imports.
