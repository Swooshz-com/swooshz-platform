...
The next likely platform PR should wire a live provider configuration behind the existing injected OIDC boundary before broadening browser routes. Frontend shell work should still wait until backend auth, session, persistence, CSRF, and app-access boundaries are stable.
