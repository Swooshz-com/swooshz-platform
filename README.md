...
The repo now includes a platform-only app launch intent contract: `POST /api/platform/apps/launch?workspaceId=...&appKey=...`. The route requires the browser session cookie plus Origin/Referer and CSRF token validation, re-checks protected app access with the existing entitlement and role rules, and creates a short-lived launch token record that stores only an HMAC hash. The raw launch token is returned only once in the immediate no-store response along with app key, workspace id, optional app launch URL, and expiry. `APP_LAUNCH_TOKEN_HASH_SECRET` is required when runtime/bootstrap composes launch issuing dependencies. This PR does not add launch-token consumption/validation, KQAG storage or launch adapters, app launch redirects, frontend UI, fake login, provider SDKs, billing, deployment, or migration execution.
