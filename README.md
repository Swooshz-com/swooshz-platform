...
The repo now includes platform-only internal workspace/app-access seed contracts. The seed service can idempotently prepare an internal workspace, the `kqag` app registry record, the workspace entitlement, and an owner/admin/member membership grant through storage-agnostic repository ports. It can grant access to an existing authenticated user by user id or normalized email, or create a user and provider identity together only when provider key, provider subject, and verified email are all explicitly supplied. It refuses email-only user precreation for future provider linking, fake-login shortcuts, conflicting records, and viewer grants for KQAG.
