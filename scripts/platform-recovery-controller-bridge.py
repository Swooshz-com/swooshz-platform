...
CANONICAL_LOADER_PAYLOAD_BYTES = (
    ...
    b"_CAPABILITY_CLASSIFICATION_ROOTS = (\"os\", \"contextlib\", \"signal\", \"subprocess\", \"sys\")\n"
    ...
    b"})\n"
    b"\n"
    b"_CAPABILITY_OS_CLASSIFICATION_DENY_NAMES = frozenset(\n"
    b"    \"system popen startfile fork forkpty vfork posix_spawn posix_spawnp \"\n"
    b"    \"execl execle execlp execlpe execv execve execvp execvpe spawnl spawnle \"\n"
    b"    \"spawnlp spawnlpe spawnv spawnve spawnvp spawnvpe P_WAIT P_NOWAIT P_NOWAITO \"\n"
    b"    \"P_OVERLAY P_DETACH P_DETACHED dup dup2 dup3 pipe pipe2 openpty posix_openpt \"\n"
    b"    \"grantpt unlockpt login_tty get_inheritable set_inheritable get_handle_inheritable \"\n"
    b"    \"set_handle_inheritable get_blocking set_blocking pidfd_getfd O_CLOEXEC O_CLOFORK \"\n"
    b"    \"O_NOINHERIT register_at_fork\".split()\n"
    b")\n"
    b"_CAPABILITY_OS_RETAIN_SAFE_NAMES = frozenset(\n"
    b"    \"abc abort access add_dll_directory alarm altsep chdir chmod chown chroot \"\n"
    b"    \"CLD_CONTINUED CLD_DUMPED CLD_EXITED CLD_KILLED CLD_STOPPED CLD_TRAPPED close \"\n"
    b"    \"closerange confstr confstr_names copy_file_range cpu_count ctermid curdir defpath \"\n"
    b"    \"device_encoding devnull DirEntry environ error eventfd eventfd_read eventfd_write \"\n"
    b"    \"EX__BASE EX_CANTCREAT EX_CONFIG EX_DATAERR EX_IOERR EX_NOHOST EX_NOINPUT EX_NOMSG \"\n"
    b"    \"EX_NOPERM EX_NOUSER EX_OK EX_OSERR EX_OSFILE EX_PROTOCOL EX_SOFTWARE EX_TEMPFAIL \"\n"
    b"    \"EX_UNAVAILABLE EX_USAGE extsep F_OK fchmod fchmodat fchown fchownat fdatasync \"\n"
    b"    \"fdopen fpathconf fsdecode fsencode fspath fstat fstatvfs fsync ftruncate fwalk \"\n"
    b"    \"GenericAlias get_exec_path get_terminal_size getcwd getcwdb getegid getenv geteuid \"\n"
    b"    \"getgid getgrouplist getgroups getloadavg getlogin getpgid getpgrp getpid getppid \"\n"
    b"    \"getpriority getrandom getsid getuid if_indextoname if_nameindex if_nametoindex \"\n"
    b"    \"initgroups isatty kill killpg lchmod lchown linesep link listdir listdrives \"\n"
    b"    \"listmounts listvolumes listxattr lockf lseek lstat major makedev makedirs Mapping \"\n"
    b"    \"memfd_create mkdir mkfifo mknod MutableMapping name nice O_APPEND O_ASYNC O_BINARY \"\n"
    b"    \"O_CREAT O_DIRECT O_DIRECTORY O_DSYNC O_EXCL O_LARGEFILE O_NOATIME O_NOCTTY \"\n"
    b"    \"O_NOFOLLOW O_NONBLOCK O_PATH O_RANDOM O_RDONLY O_RDWR O_RSYNC O_SEQUENTIAL \"\n"
    b"    \"O_SHORT_LIVED O_SYNC O_TEMPORARY O_TEXT O_TMPFILE O_TRUNC O_WRONLY open pardir path \"\n"
    b"    \"pathconf pathconf_names PathLike pathsep pidfd_open POSIX_FADV_DONTNEED \"\n"
    b"    \"POSIX_FADV_NOREUSE POSIX_FADV_NORMAL POSIX_FADV_RANDOM POSIX_FADV_SEQUENTIAL \"\n"
    b"    \"POSIX_FADV_WILLNEED posix_fadvise posix_fallocate POSIX_SPAWN_CLOSE \"\n"
    b"    \"POSIX_SPAWN_DUP2 POSIX_SPAWN_OPEN pread preadv process_cpu_count putenv pwrite \"\n"
    b"    \"pwritev R_OK read readinto readlink readv realpath reload_environ remove removedirs \"\n"
    b"    \"removexattr rename renames replace rmdir RTLD_DEEPBIND RTLD_GLOBAL RTLD_LAZY \"\n"
    b"    \"RTLD_LOCAL scandir sched_get_priority_max sched_get_priority_min sched_getaffinity \"\n"
    b"    \"sched_getparam sched_getscheduler sched_rr_get_interval sched_setaffinity \"\n"
    b"    \"sched_setparam sched_setscheduler SEEK_CUR SEEK_END SEEK_SET sendfile sep setegid \"\n"
    b"    \"seteuid setgid setgroups setns setpgid setpgrp setsid setuid setxattr st stat \"\n"
    b"    \"stat_result statvfs_result strerror supports_bytes_environ supports_dir_fd \"\n"
    b"    \"supports_effective_ids supports_fd supports_follow_symlinks symlink sync sys sysconf \"\n"
    b"    \"sysconf_names tcgetpgrp tcsetpgrp terminal_size times times_result TMP_MAX truncate \"\n"
    b"    \"ttyname umask uname uname_result unlink unsetenv urandom utime W_OK wait wait3 wait4 \"\n"
    b"    \"waitid waitpid waitstatus_to_exitcode walk WCONTINUED WCOREDUMP WEXITED \"\n"
    b"    \"WIFCONTINUED WIFEXITED WIFSIGNALED WIFSTOPPED WNOHANG WNOWAIT write WSTOPPED WUNTRACED writev X_OK\".split()\n"
    b")\n"
    b"_CAPABILITY_CONTEXTLIB_CLASSIFICATION_DENY_NAMES = frozenset(\n"
    b"    \"redirect_stdin redirect_stdout redirect_stderr\".split()\n"
    b")\n"
    b"_CAPABILITY_CONTEXTLIB_RETAIN_SAFE_NAMES = frozenset(\n"
    b"    \"abc AbstractAsyncContextManager AbstractContextManager aclosing \"\n"
    b"    \"AsyncContextDecorator asynccontextmanager AsyncExitStack chdir closing ContextDecorator \"\n"
    b"    \"contextmanager deque ExitStack GenericAlias MethodType nullcontext os suppress sys wraps\".split()\n"
    b")\n"
    b"_CAPABILITY_SIGNAL_CLASSIFICATION_DENY_NAMES = frozenset(\n"
    b"    \"set_wakeup_fd\".split()\n"
    b")\n"
    b"_CAPABILITY_SIGNAL_RETAIN_SAFE_NAMES = frozenset(\n"
    b"    \"alarm CTRL_BREAK_EVENT CTRL_C_EVENT default_int_handler getsignal Handlers \"\n"
    b"    \"ITIMER_PROF ITIMER_REAL ITIMER_VIRTUAL NSIG pause pthread_kill pthread_sigmask \"\n"
    b"    \"raise_signal SIG_DFL SIG_IGN SIGABRT SIGALRM SIGBREAK SIGBUS SIGCHLD SIGCONT SIGFPE \"\n"
    b"    \"SIGILL SIGINT siginterrupt SIGIO SIGIOT SIGKILL signal Signals sigpending SIGPIPE \"\n"
    b"    \"SIGPOLL SIGPROF SIGPWR SIGQUIT SIGRTMAX SIGRTMIN SIGSEGV SIGSTKFLT SIGSTOP SIGSYS \"\n"
    b"    \"SIGTERM sigtimedwait SIGTSTP SIGTTIN SIGTTOU SIGURG SIGUSR1 SIGVTALRM sigwait \"\n"
    b"    \"sigwaitinfo SIGWINCH SIGXCPU SIGXFSZ strsignal valid_signals\".split()\n"
    b")\n"
    b"_CAPABILITY_SUBPROCESS_GUARDED_NAMES = frozenset(\n"
    b"    \"PIPE DEVNULL STDOUT TimeoutExpired CompletedProcess Popen run check_output\".split()\n"
    b")\n"
    b"_CAPABILITY_SYS_GUARDED_NAMES = frozenset(\n"
    b"    \"executable stdin stdout stderr __stdin__ __stdout__ __stderr__\".split()\n"
    b")\n"
    b"\n"
    b"def _capability_classification_map(denied=(), retained=(), guarded=()):\n"
    b"    entries = {name: _CAPABILITY_DENY for name in denied}\n"
    b"    entries.update({name: _CAPABILITY_RETAIN_SAFE for name in retained})\n"
    b"    entries.update({name: _CAPABILITY_GUARDED_SAFE for name in guarded})\n"
    b"    return _types.MappingProxyType(entries)\n"
    b"\n"
    b"def _capability_platform_map():\n"
    b"    return _types.MappingProxyType({\n"
    b"        \"os\": _capability_classification_map(\n"
    b"            _CAPABILITY_OS_CLASSIFICATION_DENY_NAMES,\n"
    b"            _CAPABILITY_OS_RETAIN_SAFE_NAMES,\n"
    b"        ),\n"
    b"        \"contextlib\": _capability_classification_map(\n"
    b"            _CAPABILITY_CONTEXTLIB_CLASSIFICATION_DENY_NAMES,\n"
    b"            _CAPABILITY_CONTEXTLIB_RETAIN_SAFE_NAMES,\n"
    b"        ),\n"
    b"        \"signal\": _capability_classification_map(\n"
    b"            _CAPABILITY_SIGNAL_CLASSIFICATION_DENY_NAMES,\n"
    b"            _CAPABILITY_SIGNAL_RETAIN_SAFE_NAMES,\n"
    b"        ),\n"
    b"        \"subprocess\": _capability_classification_map(\n"
    b"            guarded=_CAPABILITY_SUBPROCESS_GUARDED_NAMES,\n"
    b"        ),\n"
    b"        \"sys\": _capability_classification_map(\n"
    b"            guarded=_CAPABILITY_SYS_GUARDED_NAMES,\n"
    b"        ),\n"
    b"    })\n"
    b"\n"
    b"_CAPABILITY_CLASSIFICATION_REGISTRY = _types.MappingProxyType({\n"
    b"    \"nt\": _capability_platform_map(),\n"
    b"    \"posix\": _capability_platform_map(),\n"
    b"})\n"
    ...
    b"    if root not in _CAPABILITY_CLASSIFICATION_ROOTS:\n"
    b"        return None\n"
    b"    registry = _capability_registry()\n"
    ...
    b"    def __getattribute__(self, name):\n"
    b"        if not isinstance(name, str):\n"
    b"            raise AttributeError(name)\n"
    b"        root = object.__getattribute__(self, \"_root\")\n"
    b"        classification = _capability_classification(root, name)\n"
    b"        if name.startswith(\"_\") and classification != _CAPABILITY_GUARDED_SAFE:\n"
    b"            raise AttributeError(name)\n"
    b"        if (\n"
    b"            (root in _CAPABILITY_CLASSIFICATION_ROOTS and classification is None)\n"
    b"            or classification == _CAPABILITY_DENY\n"
    b"            or name in _PROCESS_CAPABILITY_POLICY.get(root, ())\n"
    b"        ):\n"
    b"            raise AttributeError(name)\n"
    b"        if classification == _CAPABILITY_GUARDED_SAFE:\n"
    b"            if root == \"sys\":\n"
    b"                return getattr(_GUARDED_SYS, name)\n"
    b"            if root == \"subprocess\":\n"
    b"                return getattr(_GUARDED_SUBPROCESS, name)\n"
    b"            raise AttributeError(name)\n"
    ...
    b"            return _guard_module_alias(value, child_root)\n"
    b"        return value\n"
    b"    def __setattr__(self, _name, _value):\n"
    ...
    b"        module = object.__getattribute__(self, \"_module\")\n"
    b"        root = object.__getattribute__(self, \"_root\")\n"
    b"        if root in _CAPABILITY_CLASSIFICATION_ROOTS:\n"
    b"            entries = _capability_registry()[root]\n"
    b"            names = []\n"
    b"            for name, classification in entries.items():\n"
    b"                if classification not in (_CAPABILITY_RETAIN_SAFE, _CAPABILITY_GUARDED_SAFE):\n"
    b"                    continue\n"
    b"                try:\n"
    b"                    value = getattr(module, name)\n"
