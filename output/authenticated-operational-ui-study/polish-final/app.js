(() => {
  ...
  const launchStatusDot = document.querySelector("#launch-status-dot");
  const setLaunchState = state => {
    launchReadiness?.classList.remove("is-loading", "is-unavailable");
    launchFeedback.hidden = true;
    launchButton.disabled = false;
    launchButton.querySelector("span").textContent = "Launch product";
    if (launchStatus) launchStatus.textContent = "Ready to launch";
    launchStatusDot?.classList.remove("is-disabled");
    if (state === "loading") {
      launchReadiness.classList.add("is-loading");
      launchButton.disabled = true;
      launchButton.querySelector("span").textContent = "Opening product...";
      if (launchStatus) launchStatus.textContent = "Opening product";
      announce("Opening the product.");
    } else if (state === "failure") {
      if (launchStatus) launchStatus.textContent = "Could not open";
      launchFeedback.hidden = false;
      launchFeedback.innerHTML = '<strong>We could not open the product. Try again.</strong><button type="button" data-retry-launch>Retry</button>';
      launchFeedback.querySelector("button").addEventListener("click", launchSequence);
      announce("We could not open the product. Retry is available.");
    } else if (state === "unavailable") {
      launchReadiness.classList.add("is-unavailable");
      launchButton.disabled = true;
      launchButton.querySelector("span").textContent = "Launch product";
      if (launchStatus) launchStatus.textContent = "Product access unavailable";
      launchStatusDot?.classList.add("is-disabled");
      launchFeedback.hidden = false;
      launchFeedback.innerHTML = '<strong>Product access unavailable</strong>Your workspace does not currently have access to this product.';
    }
  };
  ...
  addForm.addEventListener("submit", async event => {
    ...
    if (email.endsWith("fail.invalid")) {
      ...
      modalFeedback.textContent = "We could not add this member. Check the email address and try again.";
      announce("We could not add this member.");
      return;
    }
  ...
  const confirmationCopy = {
    "disable-product": ["Disable product access?", "Members will be unable to launch this product until access is enabled again.", "Disable product access"],
    ...
    "role": ["Change this member's role?", "Confirm that you want to change this member's role.", "Change role"]
  };
