...
class ManagedController:
    ...
    def _run_supervised_agent(self, *, agent_binary: Path, source: Path, target: Path, result_payload: bytes,
        ...
        try:
            frame = BACKEND.decode_frame(received)
            parsed = BACKEND.parse_managed_json(frame.payload)
        except Exception as error:
            raise ControllerError("agent-result-invalid") from error
        if frame.message != "RESULT" or frame.direction != BACKEND.DIRECTION_REMOTE_TO_LOCAL or frame.sequence != 9 or frame.payload != result_payload or not isinstance(parsed, list):
    ...
    def run(self, *, store_root: Path, artifact_source: Path, artifact_target: Path, agent_binary: Path,
        ...
        discovery_payload, _ = BACKEND.build_discovery(_raw_hex(self.context.session_hex, "session"), execution_row_id, filename,
        ...
        discovery_frame = BACKEND.build_frame(BACKEND.DIRECTION_LOCAL_TO_REMOTE, "DISCOVERY", 6, _frame_nonce_from(discovery_nonce), discovery_payload)
        restore_begin_payload = BACKEND.build_restore_begin(_raw_hex(self.context.session_hex, "session"), BACKEND.frame_hash(discovery_frame), transition_wire, evidence_wire, consumed_record_commitment)
        restore_begin_frame = BACKEND.build_frame(BACKEND.DIRECTION_REMOTE_TO_LOCAL, "RESTORE_BEGIN", 7, _frame_nonce_from(restore_begin_nonce), restore_begin_payload)
        proceed_payload, proceed_commitment = BACKEND.build_proceed(_raw_hex(self.context.session_hex, "session"), transition_identifier, transition_data_commitment, BACKEND.frame_hash(restore_begin_frame))
        proceed_frame = BACKEND.build_frame(BACKEND.DIRECTION_LOCAL_TO_REMOTE, "PROCEED", 8, _frame_nonce_from(proceed_nonce), proceed_payload)
        ...
        result_payload, result_commitment = BACKEND.build_result(_raw_hex(self.context.session_hex, "session"), transition_identifier,
