...
class AdmissionContext:
    ...
    def validate(self) -> None:
        ...
        if any(value == "0" * 64 for value in (
            self.session_hex,
            self.generation_hex,
            self.accepted_connection_hex,
            self.connection_cookie_hex,
        )):
...
class ManagedController:
    ...
    def _run_supervised_agent(
        ...
        *,
        ...
        proceed_frame: bytes,
        agent_exchange: Callable[[bytes, bytes, Path, Path, bytes], bytes] | None,
    ) -> None:
        """Use only a provider that has already completed the supervisor proof.

        This orchestration layer never substitutes a PID, environment value, or
        executable path for the kernel-backed registration. The provider owns
        the real supervisor/custodian/OpenSSH boundary and returns one exact
        RESULT frame after its one-use stream has reached EOF.
        """
        ...
        context_record = (
            b"SWZCTX01"
            + bytes.fromhex(self.context.session_hex)
            + bytes.fromhex(self.context.generation_hex)
            + bytes.fromhex(self.context.accepted_connection_hex)
            + bytes.fromhex(self.context.connection_cookie_hex)
        )
    ...
    def run(
        ...
        *,
        ...
        agent_exchange: Callable[[bytes, bytes, Path, Path, bytes], bytes] | None = None,
    ) -> SessionResult:
        if not self.accepted:
            self.accept()
        ...
        prepared_store_frame = store.prepare_runner_frame(
            epoch_ref,
            "RESTORE_BEGIN",
            {"ref": transition_id, "commitment": transition_data_commitment},
        )
...
def make_admitted_context(
    ...
    generation_hex: str,
    accepted_connection_hex: str,
    connection_cookie_hex: str | None = None,
) -> AdmissionContext:
    if connection_cookie_hex is None:
        accepted = _raw_hex(accepted_connection_hex, "accepted-connection")
        connection_cookie_hex = hashlib.sha256(
            b"swz-qualification-cookie.v1" + accepted
        ).hexdigest()
    context = AdmissionContext(
        session_hex,
        generation_hex,
        accepted_connection_hex,
        connection_cookie_hex,
        True,
        True,
    )
    context.validate()
