...
def build_openssh(lock: dict[str, Any], output: Path, cache: Path) -> dict[str, str]:
    ...
    staged_client = staged_prefix / "bin" / "ssh"
    ...
    client_installed = output / "openssh" / "bin" / "ssh"
    keygen_installed = output / "openssh" / "bin" / "ssh-keygen"
...
def write_manifest(output: Path, lock: dict[str, Any], openssh: dict[str, str], musl: dict[str, str], native: dict[str, str]) -> Path:
    ...
    manifest = {
        ...
        "openssh": {"version": "OpenSSH_10.5p1", "archive_sha256": openssh.get("archive_sha256", ""), "patch_sha256": openssh.get("patch_sha256", ""), "binary_sha256": openssh.get("binary_sha256", ""), "ssh_sha256": openssh.get("client_sha256", ""), "ssh_keygen_sha256": openssh.get("keygen_sha256", ""), "ssh_agent_sha256": "", "ssh_add_sha256": "", "sshd_version": openssh.get("version", ""), "host_key": "/etc/ssh/recovery_host_ed25519_key.pub", "host_key_agent": "/run/swz/recovery-hostkey-agent.sock", "session_control": "/run/swz/recovery-session-control.sock"},
