"""Reproducible disposable build; stock agent artifacts are never staged for runtime."""
...
def build_openssh(lock: dict[str, Any], output: Path, cache: Path) -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="swz-openssh-") as temporary:
        ...
        agent_digest = sha256_file(agent)
        add_digest = sha256_file(add)
    ...
    return {
        ...
        "ssh_agent_sha256": agent_digest,
        "ssh_add_sha256": add_digest,
        "sshd_version": version,
        "host_key": "/etc/ssh/recovery_host_ed25519_key.pub",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.native_only:
            ...
            print(json.dumps({"status": "PASS", "native": native}, sort_keys=True))
            return 0
