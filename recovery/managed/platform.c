...
int swz_send_agent_descriptor_bundle(int socket_fd, const int fds[3])
{
    struct msghdr message;
    struct iovec vector;
    unsigned char record[SWZ_AGENT_BUNDLE_RECORD_BYTES];
    union {
        struct cmsghdr header;
        unsigned char bytes[CMSG_SPACE(3U * sizeof(int))];
    } control;
    struct cmsghdr *header;

    if (socket_fd < 0 || fds == NULL || fds[0] < 0 || fds[1] < 0 ||
        fds[2] < 0 || fds[0] == fds[1] || fds[0] == fds[2] ||
        fds[1] == fds[2]) {
        return -1;
    }
    memset(record, 0, sizeof(record));
    memcpy(record, SWZ_AGENT_BUNDLE_MAGIC,
           sizeof(SWZ_AGENT_BUNDLE_MAGIC) - 1U);
    record[27U] = (unsigned char)SWZ_RESULT_CONTEXT_VERSION;
    record[28U] = 3U;
    record[29U] = 1U;
    record[30U] = 2U;
    record[31U] = 3U;
    ...
    header->cmsg_level = SOL_SOCKET;
    header->cmsg_type = SCM_RIGHTS;
    header->cmsg_len = CMSG_LEN(3U * sizeof(int));
    memcpy(CMSG_DATA(header), fds, 3U * sizeof(int));
...
static void close_agent_bundle_rights(const struct msghdr *message)
{
    struct cmsghdr *header;
    ...
    for (header = CMSG_FIRSTHDR((struct msghdr *)message); header != NULL;
        ...
        data_length = header->cmsg_len - CMSG_LEN(0U);
        available_length = message->msg_controllen > CMSG_LEN(0U)
                               ? message->msg_controllen - CMSG_LEN(0U)
                               : 0U;
        if (data_length > available_length) {
            data_length = available_length;
        }
        count = data_length / sizeof(int);
        for (index = 0U; index < count; ++index) {
            int received_fd;

            memcpy(&received_fd, (const unsigned char *)CMSG_DATA(header) +
                       (index * sizeof(received_fd)), sizeof(received_fd));
            close(received_fd);
        }
    }
}
...
int swz_recv_agent_descriptor_bundle(int socket_fd, int fds[3])
{
    ...
    message.msg_control = control.bytes;
    message.msg_controllen = sizeof(control.bytes);
    received_bytes = recvmsg(socket_fd, &message, MSG_CMSG_CLOEXEC | MSG_TRUNC);
    for (header = CMSG_FIRSTHDR(&message); header != NULL;
         header = CMSG_NXTHDR(&message, header)) {
        if (header->cmsg_level != SOL_SOCKET ||
            header->cmsg_type != SCM_RIGHTS ||
            header->cmsg_len != CMSG_LEN(3U * sizeof(int)) ||
            rights_count != 0U) {
            close_agent_bundle_rights(&message);
            return -1;
        }
        memcpy(received, CMSG_DATA(header), 3U * sizeof(int));
        rights_count = 3U;
    }
    if (received_bytes != (ssize_t)sizeof(record) ||
        (message.msg_flags & (MSG_CTRUNC | MSG_TRUNC)) != 0 ||
        memcmp(record, SWZ_AGENT_BUNDLE_MAGIC,
               sizeof(SWZ_AGENT_BUNDLE_MAGIC) - 1U) != 0 ||
        record[27U] != (unsigned char)SWZ_RESULT_CONTEXT_VERSION ||
        record[28U] != 3U || record[29U] != 1U || record[30U] != 2U ||
        record[31U] != 3U || rights_count != 3U || received[0] < 0 || received[1] < 0 ||
        received[2] < 0 || received[0] == received[1] ||
        received[0] == received[2] || received[1] == received[2]) {
        close_agent_bundle_rights(&message);
        return -1;
    }
