...
def run_c_native_kat(build_output: Path) -> dict[str, str]:
    ...
    backend = load_module("swz_c_python_wire_kat", HERE / "backend.py")
    python_frames = backend.wire_kat_frames()
    ...
    if tuple(backend.frame_hash(frame).hex() for frame in python_frames) != expected_frame_hashes:
        raise CandidateDefect("python-wire-kat")
    expected = {
        ...
        "boot_frame_hash": expected_frame_hashes[0],
        "challenge_frame_hash": expected_frame_hashes[1],
        "evidence_frame_hash": expected_frame_hashes[2],
        "accept_frame_hash": expected_frame_hashes[3],
        "accepted_frame_hash": expected_frame_hashes[4],
    }
    labels = ("boot", "challenge", "evidence", "accept", "accepted")
    expected.update({f"{label}_frame_hex": frame.hex()
                     for label, frame in zip(labels, python_frames)})
    ...
    return {"status": "PASS", "complete_frame_byte_equality": "PASS", **expected}
...
def ensure_disposable_locator_client_path(container: str) -> None:
    script = (
        "target=\"$(command -v psql)\" && test -n \"$target\" && test -x \"$target\" && "
        "if test -e /usr/local/bin/psql || test -L /usr/local/bin/psql; then "
        "test -L /usr/local/bin/psql && test \"$(readlink -f -- /usr/local/bin/psql)\" = \"$target\"; "
        "else ln -s -- \"$target\" /usr/local/bin/psql; fi && "
        "test \"$(readlink -f -- /usr/local/bin/psql)\" = \"$target\" && "
        "exec /usr/local/bin/psql -X -w -n -q -A -t -v ON_ERROR_STOP=1 "
        "--host=/var/run/postgresql --port=5432 --dbname=coolify --pset=pager=off -f -"
    )
    command = ["docker", "exec", "--user", "root", container, "/bin/sh", "-c", script]
...
INSERT INTO scheduled_database_backup_executions VALUES (23, 1, 'coolify', 'success', '2026-09-07T01:00:00Z', '830082', TRUE, 'qualified-artifact', FALSE);"""
        ...
        setup = run(["docker", "exec", "-i", container, "/usr/local/bin/psql", "-U", "postgres", "-d", "coolify", "-v", "ON_ERROR_STOP=1"], input_text=setup_sql, timeout=30)
...
def run_store_cas_locator_integration(build_output: Path) -> dict[str, Any]:
    ...
    runner.accept(controller.BACKEND.wire_kat_frames(
        runner.n_local,
        session=bytes.fromhex(context.session_hex),
        connection=bytes.fromhex(context.accepted_connection_hex),
    )[-1])
    agent_binary = build_output / "native" / "swz-agent"

    def agent_exchange(context_record: bytes, proceed_frame: bytes, source: Path,
                       target: Path, result_payload: bytes) -> bytes:
        expected_context = (b"SWZCTX01" + bytes.fromhex(context.session_hex) +
                            bytes.fromhex(context.generation_hex) +
                            bytes.fromhex(context.accepted_connection_hex) +
                            bytes.fromhex(context.connection_cookie_hex))
        if context_record != expected_context or len(context_record) != 136:
            raise RuntimeError("context-record-mismatch")
        source_fd = os.open(source, os.O_RDONLY | getattr(os, "O_CLOEXEC", 0))
        target_fd = os.open(target, os.O_RDWR | os.O_CREAT | os.O_TRUNC |
                            getattr(os, "O_CLOEXEC", 0), 0o600)
        child_source_fd = 100
        child_target_fd = 101
        os.dup2(source_fd, child_source_fd, inheritable=True)
        os.dup2(target_fd, child_target_fd, inheritable=True)

        def inherit_agent_fds() -> None:
            os.dup2(child_source_fd, 4)
            os.dup2(child_target_fd, 5)

        try:
            process = subprocess.Popen(
                [str(agent_binary)], stdin=subprocess.PIPE,
                stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                pass_fds=(child_source_fd, child_target_fd), preexec_fn=inherit_agent_fds,
            )
            stdout, stderr = process.communicate(proceed_frame, timeout=30)
        finally:
        ...
        if process.returncode != 0 or stdout or stderr:
            raise RuntimeError("native-agent-exchange-failed")
        proceed = controller.BACKEND.decode_frame(
            proceed_frame, expected_n_local=runner.n_local,
            expected_direction=controller.BACKEND.DIRECTION_LOCAL_TO_REMOTE,
            expected_message="PROCEED", expected_sequence=7,
        )
        result_frame = controller.BACKEND.build_frame(
            controller.BACKEND.DIRECTION_REMOTE_TO_LOCAL, "RESULT", 8,
            proceed.n_local, result_payload,
        )
        return result_frame
    ...
    with tempfile.TemporaryDirectory(prefix="swz-store-locator-") as temporary:
        ...
        runner.run(store_root=store_root, artifact_source=source,
                   artifact_target=target,
                   agent_binary=build_output / "native" / "swz-agent",
                   agent_exchange=agent_exchange)
        if (runner.store is None or
                runner.store.read_restore_ledger("epoch-qualified-001")["state"] != "CONSUMED" or
                not target.is_file() or target.read_bytes() != source.read_bytes()):
            raise CandidateDefect("cas-consumption-not-proven")
        return {"status": "PASS", "cas": "CONSUMED_AND_RESTORED",
...
def run_guest(build_output: Path, output: Path) -> dict[str, Any]:
    ...
    if result.returncode == 2:
        raise ProviderHold(_bounded(result.stderr))
    if result.returncode != 0:
