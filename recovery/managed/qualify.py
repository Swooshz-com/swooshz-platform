...
def run_deterministic_tests() -> dict[str, Any]:
    ...
    if result.returncode != 0:
        if os.name == "nt":
            ...
            if "SECURITY_PROOF_FAILED" in diagnostics:
                raise ProviderHold("LOCAL_CAPABILITY_UNAVAILABLE:windows-store-security-proof")
            if "No usable temporary directory" in diagnostics:
                raise ProviderHold("LOCAL_CAPABILITY_UNAVAILABLE:windows-temporary-directory")
...
def run_result_ownership_static() -> dict[str, str]:
    controller_source = (HERE / "controller.py").read_text(encoding="utf-8")
    qualify_source = (HERE / "qualify.py").read_text(encoding="utf-8")
    if any(token in controller_source for token in ("result_" + "payload", "BACKEND." + "build_result", "pre" + "exec_fn")):
        raise CandidateDefect("python-result-fabrication-path")
    if any(token in qualify_source for token in ("BACKEND." + "build_result", "pre" + "exec_fn", "dup" + "2")):
        raise CandidateDefect("qualification-result-fabrication-path")
    if "NativeAgentExchange" not in controller_source:
        raise CandidateDefect("native-result-exchange-missing")
    return {"status": "PASS", "PYTHON_FABRICATED_RESULT": "ABSENT"}
    ...
    with tempfile.TemporaryDirectory(prefix="swz-store-locator-") as temporary:
        ...
        try:
            runner.run(store_root=store_root, artifact_source=source,
                       ...
                       agent_exchange=None)
        except controller.QualificationProviderHold as error:
            ...
            if str(error) != "SECURITY_PROOF_FAILED":
                raise
            raise ProviderHold("LOCAL_CAPABILITY_UNAVAILABLE:windows-store-security-proof") from error
        raise CandidateDefect("native-provider-returned-without-runtime-result")
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence: dict[str, Any] = {"schema": "swz-managed-qualification-evidence.v1", "phase": args.phase, "candidate_sha": args.expected_sha, "mandatory_security_skips": "NOT_YET_PROVEN", "failures": []}
    ...
    try:
        ...
        evidence["result_ownership"] = run_result_ownership_static()
