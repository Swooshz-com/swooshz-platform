"""Validate and materialize a disposable managed-generation install plan.

This command writes a plan and verifies source digests.  It never installs,
activates or publishes a generation.
"""
...
def build_plan(manifest_path: Path) -> dict[str, Any]:
    ...
    entries.extend(
        [
            ...
            {
                "source": checked_source(root, "openssh/bin/ssh-agent", openssh["ssh_agent_sha256"]),
                "destination": "/opt/swz/openssh/bin/ssh-agent",
            },
            {
                "source": checked_source(root, "openssh/bin/ssh-add", openssh["ssh_add_sha256"]),
                "destination": "/opt/swz/openssh/bin/ssh-add",
            },
            {
