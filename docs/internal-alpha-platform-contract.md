...
| What happens when a user is removed? | The workspace membership row is removed, the removed user's active platform sessions are revoked, new session context should omit that workspace, app launch should fail closed, future sign-in may create a new session only through the provider-backed path, and audit events should record the action. If the user is disabled globally, new app launch must fail for all workspaces. |
