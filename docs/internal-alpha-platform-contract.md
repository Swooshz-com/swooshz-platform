...
| Removing/deactivating user | Implemented partial | Partial admin surface shipped; global user disable remains a future decision | Protected admin HTTP routes and `/app/admin` can disable a workspace membership and reactivate a disabled non-owner membership with audit events, last-owner protection, self-removal guard, and owner-reactivation protection. | Decide whether global user disable is needed for alpha. |
...
- Role changes, membership disables, membership reactivations, and KQAG entitlement toggles use the existing CSRF token route and send the required `x-csrf-token` header to the protected routes.
