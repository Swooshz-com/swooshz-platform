...
| Workspace model | `src/accounts/types.ts`, `src/db/schema.ts`, `docs/accounts-contract.md` | Workspaces have id, slug, display name, and status. Membership connects users to workspaces with admin/operator/viewer roles and active/disabled status. Pending workspace membership approvals store normalized email, workspace, role, status, nullable actor for first-admin bootstrap only, acceptance/revocation timestamps, and no invitation token material. Admin-created teammate approvals require a non-null requester except for the single first-admin bootstrap approval. |
| App access model | `src/apps/types.ts`, `src/access/decide-app-access.ts`, `src/platform/app-access-service.ts`, `docs/app-access-contract.md` | App launch requires valid session, active user, selected active workspace, active membership, available/private-preview app, enabled/trial entitlement, and role permission. SQAG launch is admin/operator only. |
| Admin service foundation | `src/platform/workspace-admin-service.ts`, `src/platform/repositories.ts`, `src/db/repositories.ts`, `src/http/handlers.ts`, `src/http/node-adapter.ts` | Admin service methods and protected HTTP routes can list workspace members, add existing active provider-backed users by normalized email, create/list/revoke pending workspace membership approvals, change roles, disable and reactivate other memberships, list app entitlements, and enable/disable SQAG app entitlement. They can also browse recent workspace audit events. Mutations preserve last-admin/self-change guardrails and write privacy-minimized audit events in the same transaction/unit-of-work. |
...
| Seed scripts | `scripts/platform-seed-internal-access.mjs`, `src/platform/internal-access-seed-service.ts` | The seed CLI can prepare a fresh hosted DB by creating reviewed workspace/app entitlement records plus a pending first-admin approval, then real OIDC sign-in creates/links the provider-backed user and activates the approval. Without first-admin bootstrap mode, it can grant admin/operator/viewer SQAG access to an existing active provider-backed platform user after explicit confirmation. It refuses email-only user precreation and users without provider identity records. |
...
| UAT scripts/runbooks | `docs/internal-platform-smoke-runbook.md`, `docs/auth0-passwordless-email-otp-runbook.md`, `scripts/platform-start.mjs` | UAT covers explicit env, migration, start, passwordless email OTP sign-in, seed, `/app`, separate HTTPS origins, and header-only finalization. It uses synthetic placeholders and forbids secrets/private data in docs. |
| Hosted alpha runbooks/tooling | `docs/hosted-internal-alpha-runbook.md`, `docs/hosted-internal-alpha-operator-decisions.md`, `scripts/platform-readiness-check.mjs`, `scripts/platform-db-readiness-check.mjs` | The hosted deployment runbook and smoke checklist are now documented with the canonical Platform apex, redirect-only `www`, exact OIDC callback, and canonical SQAG origin while keeping private values as placeholders. The dry-run readiness checker enforces those exact production origins, valid Postgres-shaped `DATABASE_URL`, value-safe output, and no migration/server/network imports. The DB readiness checker is a separate operator command that connects to hosted Postgres and reports only sanitized config, reachability, schema, and migration status. Actual hosted deployment execution still requires reviewed infra/operator approval. |
| Docs/runbooks | `README.md`, `docs/accounts-contract.md`, `docs/app-access-contract.md`, `docs/sqag-integration-contract.md`, `docs/auth-session-security-contract.md`, `docs/internal-alpha-go-no-go-checklist.md`, `docs/adr/*.md`, `docs/internal-platform-smoke-runbook.md`, `docs/hosted-internal-alpha-runbook.md`, `docs/hosted-internal-alpha-operator-decisions.md`, `docs/auth0-passwordless-email-otp-runbook.md` | Existing docs establish platform ownership, local UAT flow, hosted internal-alpha operations guidance, operator approval gates, the auth/session security contract and gap inventory, the consolidated go/no-go checklist, and the internal-alpha product contract/page inventory. |
...
- An operator can prepare a reviewed first-admin pending approval for a fresh hosted DB or seed an already-authenticated provider-backed user into the internal workspace and grant SQAG access.
- The `/app` shell can list the current user's active workspaces and launchable apps.
- Admin users can reach `/app/admin` from the app shell, review safe workspace/member/pending-approval/SQAG entitlement/activity state, add a teammate by email and role, list/revoke pending approvals, change member roles, disable and reactivate other memberships, and enable or disable the SQAG workspace entitlement through protected admin routes.
- SQAG launch works through an explicitly configured server handoff and separate-origin finalization; no cookie is relayed by Platform.

It is not yet a complete internal admin product. The minimal admin UI is functional internal-alpha scaffolding, not a polished dashboard. Full invitation delivery/acceptance, workspace lifecycle transfer, audit export/filtering/retention management, session-management UI, workspace-wide active-session review, revoke-other-sessions, actual hosted deployment execution, and account settings remain missing.
...
| --- | --- | --- | --- | --- |
| `admin` | Workspace administrator for internal operations. | Can add, remove, disable, reactivate, and change another membership, subject to last-admin and self-change safety. | Can grant/revoke SQAG access for the workspace. | Can launch SQAG when workspace and entitlement are active. |
| `operator` | Normal internal staff user who performs quote operations without workspace administration. | Cannot manage team members. | Cannot grant/revoke app access. | Can launch SQAG when workspace and entitlement are active. |
...
Current persisted/API role vocabulary is exactly `admin`, `operator`, and `viewer`. The migration maps historical `owner` to `admin`, historical `admin` to `admin`, historical `member` to `operator`, and historical `viewer` to `viewer`. Historical audit JSON remains untouched and old strings are evidence only.

### Access And Removal Rules

| Question | Internal-alpha contract |
| --- | --- |
| Who can add team members? | Admin can add a teammate by normalized email and role before sign-in. Existing active provider-backed Platform users are added immediately. Unknown or not-yet-provider-backed users create a pending workspace membership approval and real OIDC sign-in with the matching normalized email activates the membership. Full invitation delivery remains future scope. The current seed CLI is operator-only and remains a bootstrap fallback. |
| Who can remove team members? | Admin can remove other memberships from the workspace without deleting the platform user, provider identities, or other workspace memberships. Removal revokes the removed user's active platform sessions. Disable/reactivate remains the reversible membership deactivation path. |
| Who can change roles? | Admin after admin surfaces exist, with guardrails for last-admin protection and self-demotion. |
| Who can grant/revoke SQAG access? | Admin after app-access admin exists. Grant/revoke should mutate workspace app entitlement and write audit events. |
| Who can launch SQAG? | Admin/operator today. Viewer launch is blocked for SQAG because no read-only SQAG mode exists. Future products inherit the same blocked viewer launch default until an explicit per-product read-only policy is approved. |
...
| Removing/deactivating user | Implemented partial | Partial admin surface shipped; global user disable remains a future decision | Protected admin HTTP routes and `/app/admin` can disable a workspace membership, reactivate a disabled other membership, and remove another workspace membership with audit events, last-admin protection, self-removal guard, last-admin protection, and transaction rollback on audit failure. | Decide whether global user disable is needed for alpha. |
| Changing role | Implemented partial | Partial admin surface shipped; role changes and operator access are implemented | Protected admin HTTP routes and `/app/admin` can change membership role with audit events, last-admin protection, and self-demotion guard. | Use the explicit `operator` role for quote operators after the accepted migration. |
...
| Seeding first admin | Implemented partial | Needs hosted smoke evidence before internal alpha | Seed CLI first-admin bootstrap mode creates reviewed workspace/app entitlement records and a pending admin approval without users, provider identities, sessions, or fake login state. Real OIDC sign-in activates the admin membership transactionally. | Run only as a reviewed one-off, keep real staff emails outside repo notes, and require hosted smoke evidence before readiness changes. |
| Audit event recording and browsing for admin actions | Implemented partial | Minimal browsing shipped; export/filtering/retention remains future scope | Workspace admin services emit privacy-minimized audit events for membership add, membership role change, membership disable, membership reactivation, membership removal, SQAG entitlement enable, and SQAG entitlement disable. `/app/admin` can browse recent admin-only workspace audit events through a read-only protected route. | Keep future admin routes on the same service/audit path and add export/filtering/retention only in a separate reviewed PR. |
| Admin authorization checks | Implemented partial | Implemented for current admin routes and UI | Workspace admin services and protected HTTP routes check active session, active user, active workspace, active membership, and admin role. Browser-cookie admin mutation routes use the existing CSRF/origin security helper before mutation. | Preserve fail-closed behavior for future admin routes. |

## Security Findings

| Area | Current finding | Internal-alpha action |
| --- | --- | --- |
| Auth0/passwordless OIDC configuration | Generic OIDC config supports the accepted Auth0 authority through env, verifies state/nonce, RS256 JWKS, issuer, audience, expiry, subject, nonce, and verified email. Verified normalized email may match a persisted pending workspace-membership approval for controlled onboarding. | For hosted alpha, configure the reviewed Auth0/OIDC redirect URI to the hosted HTTPS Platform callback outside the repo. A valid identity without pending Platform approval receives safe approval-required denial; active membership and app entitlement remain separate checks. Do not use email/domain environment variables as admission authority. |
...
| SQAG server handoff | Browser-safe SQAG handoff is explicit, requires exact separate HTTPS origins, is disabled by default, and is restricted to `appKey=sqag`. | Hosted UAT must verify header-only finalization, host-only cookies, replay rejection, and live access revalidation. |
| Role/app-access checks | Launch re-checks session, user, workspace, membership, app, entitlement, and role. Viewer product launch is blocked by default, including SQAG, unless a future product-specific read-only launch policy is approved. | Preserve least-privilege viewer behavior and admin authorization checks for team/app-access admin mutations. |
...
| Audit events | Audit schema/repository exists, admin mutations emit audit events, and `/app/admin` can browse recent admin-only activity. | Export/filtering/retention workflows remain future scope. Preserve privacy-minimized summaries. |
...
- `listWorkspaceMembersForAdmin`: Admin service can list workspace members with safe user and membership summaries.
- `addWorkspaceMemberByEmail`: Admin service can add an existing active provider-backed user by normalized email as `admin`, `operator`, or `viewer`; otherwise it creates a pending workspace membership approval for the normalized email and role. Existing-user adds emit `workspace.membership.added`; pending approval creates emit `workspace.membership_approval.created`; both store only privacy-minimized audit metadata.
- `listWorkspaceMembershipApprovalsForAdmin`: Admin service can list pending workspace membership approvals for a workspace.
- `revokeWorkspaceMembershipApproval`: Admin service can revoke a pending workspace membership approval and emits `workspace.membership_approval.revoked` in the same transaction/unit-of-work.
- `changeWorkspaceMemberRole`: Admin service can change membership role with last-admin protection, self-demotion guard, and `workspace.membership.role_changed` audit event in the same transaction/unit-of-work.
- `disableWorkspaceMembership`: Admin service can disable a workspace membership with last-admin protection, self-removal guard, and `workspace.membership.disabled` audit event in the same transaction/unit-of-work.
- `reactivateWorkspaceMembership`: Admin service can reactivate a disabled other workspace membership with self-reactivation guard, and `workspace.membership.reactivated` audit event in the same transaction/unit-of-work.
- `removeWorkspaceMembership`: Admin service can remove another workspace membership with last-admin protection, self-removal guard, last-admin protection, removed-user active session revocation, and `workspace.membership.removed` audit event in the same transaction/unit-of-work.
- `listWorkspaceAppEntitlementsForAdmin`: Admin service can list workspace app entitlements with app key/name/status summaries.
- `setWorkspaceAppEntitlementStatus`: admin-only app entitlement enable/disable by registered `appKey` with `workspace.app_entitlement.enabled` or `workspace.app_entitlement.disabled` audit event in the same transaction/unit-of-work. The service is product-generic; the current HTTP/admin route remains SQAG-scoped.
- `listWorkspaceAuditEventsForAdmin`: Admin service can list recent workspace audit events newest first with safe limits and privacy-minimized metadata.
...
- `POST /api/platform/workspaces/:workspaceId/members/:membershipId/reactivate`: reactivates a disabled other workspace membership through the service foundation.
- `POST /api/platform/workspaces/:workspaceId/members/:membershipId/remove`: removes a other membership from the workspace through the service foundation and revokes the removed user's active platform sessions without deleting the platform user, provider identities, or other workspace memberships.
...
- `GET /app/admin`: minimal no-store admin shell reachable from `/app` for users with an admin workspace.
...
- The add-existing-user form posts by calling the protected add route with the existing CSRF token/header. The teammate does not need to sign in before an admin creates the pending approval, but activation still requires real provider-backed OIDC sign-in with the matching normalized email. No invitation delivery or fake provider account creation happens.
- The Pending Approvals section lists pending approvals and lets admin users revoke them through the CSRF-protected revoke route.
...
- Quote operators use the explicit `operator` role after the accepted role-collapse migration.
- For local UAT, set role to `operator` for quote operators unless the user is explicitly administering the workspace.
...
- Existing disabled memberships are not reactivated by the add route; admins use the explicit membership reactivation action in `/app/admin`.
...
- The approved compact authenticated interface exists for `/app` and `/app/admin`, including responsive launcher, members, pending approvals, product access, and audit activity surfaces. This local implementation is not hosted evidence.
- The minimal admin UI and routes use active session checks, active workspace membership checks, admin authorization, CSRF/origin validation for browser-cookie mutations, and generic user-facing errors. They do not add invitation delivery, audit export/filtering/retention, global user disable, billing, SQAG app-data editing, or Google Stitch implementation.
- The former blocker phrase "No product HTTP route or UI exists yet" is resolved for the approved `/app` launcher and `/app/admin` member/pending-approval/product-access/audit controls; full invitations and audit export/filtering/retention remain out of scope.
...
- Workspace lifecycle transfer, if separately approved.
...
- Role-collapse migration is complete; operator is now a first-class role.
...
| Hosted HTTPS SQAG URL | The implemented contract uses `https://swooshz.com` for Platform and `https://quote.swooshz.com` for SQAG. | Verify DNS, certificates, exact origins, host-only cookies, and finalization during approved hosted UAT. |
| Auth0 passwordless hosted redirect URI | Auth0 passwordless runbook and hosted runbook document the placeholder callback URI shape. | Add the exact hosted callback URI in the reviewed Auth0 configuration outside the repo. |
...
| Hosted operator approvals | Hosted operator decision record/checklist is documented with required owners, evidence placeholders, repo impact, and status placeholders. | Approve host/provider, TLS/proxy, process/container, database/backup/restore, migrations/rollback, OIDC, secrets, logs, first admin identity, pending approval process, SQAG handoff mode, cross-host session/cookie strategy, incident path, and go/no-go outside repo before execution. |
| Seeding first admin | Seed CLI can prepare a pending first-admin approval on a fresh hosted DB; hosted runbook documents seed first, then real OIDC login activation. | Hosted operator must run the reviewed first-admin bootstrap mode for the exact private value outside repo notes, then complete real OIDC login. |
...
- Hosted SQAG handoff uses separate host-only cookies and the two-stage finalization contract; readiness still requires approved hosted evidence and does not follow from local tests.

## UI And IA Requirements Before Google Stitch

| Page | Purpose | Visible data | Allowed actions | Required role | Blocker status |
| --- | --- | --- | --- | --- | --- |
| App Hub / Dashboard | Give signed-in users a workspace-aware entry to apps. | Current user, active workspace, launchable apps, access status, safe launch errors. | Launch SQAG, switch workspace when multiple workspaces exist, logout. | Any active operator or admin with app entitlement for launch; admin/operator for SQAG launch today. | Partially present as `/app`; needs product polish after contract approval. |
| Workspace Settings | Show internal workspace identity and non-secret configuration. | Workspace name, slug, status, app entitlement summary, safe metadata. | Edit display name/status only if approved later; no destructive unmanaged state. | Admin. | Minimal identity summary appears in `/app/admin`; editable settings remain future scope. |
| Team Members | Manage people in the workspace. | User display name, placeholder email, role, membership status, last login if safe, pending approval status, invitation status if used later. | Add existing provider-backed user, create/revoke pending approval, remove/deactivate/reactivate membership, change role, resend/revoke invite if invitations ship. | Admin with last-admin guardrails. | Minimal list, existing-user add, pending approval list/revoke/activation, role change, membership disable, other membership reactivation, and other membership removal are implemented in `/app/admin`; full invitations remain future scope. |
| App Access | Manage workspace entitlement to SQAG and future apps. | App key/name/status, entitlement status, who granted it if available, launch eligibility summary. | Grant/revoke/suspend SQAG access, view denied reason, future app enablement. | Admin. | Minimal SQAG entitlement list and enable/disable controls are implemented in `/app/admin`; future app support remains contract-specific. |
| Audit / Activity | Review security and access changes. | Recent workspace admin audit events: membership changes, entitlement changes, actor/target ids, timestamps, and safe metadata. | Browse recent events now; filter/export later; no mutation except retention workflow later. | Admin-only for export. | Minimal Activity browsing is implemented in `/app/admin`; export/filtering/retention remains future scope. |
| Security / Sessions | Review active sessions and security posture. | Current session, recent auth failure categories, session expiry, allowed auth provider. | Logout current session now; revoke other sessions later. | Current user for own session; admin for workspace-wide security later. | Partial logout exists; management page is future production enhancement. |
| Deployment / Health | Internal operator page or runbook-only status. | Health check, app/runtime version, SQAG handoff mode, non-secret config presence checks. | Run smoke checklist manually; no secret display. | Operator/admin, or runbook-only. | Runbook is documented; a UI remains future and actual hosted execution still requires reviewed infra/operator approval. |
| Billing / Credits | Reserve commercial concepts. | Placeholder only; no balances or payment provider ids. | None until billing is approved. | Admin later. | Future only; not an internal-alpha blocker. |
...
- The admin/operator/viewer role contract and migration are approved, with last-admin safety and viewer launch denial.
- Admin can list members, add existing provider-backed users, create/revoke pending approvals before sign-in, remove/deactivate, and change roles for workspace users; full invite delivery remains a separate decision.
- Admin can grant/revoke SQAG access.
