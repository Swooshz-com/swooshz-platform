...
SQAG has a cross-origin Platform launch path: Google-compatible generic OIDC can create a Platform session, `/app` can show workspace/app access, and Platform sends the one-time launch token only server-to-server before a separate-origin, header-only finalization at SQAG.
...
| UAT scripts/runbooks | `docs/internal-platform-smoke-runbook.md`, `docs/google-oidc-setup-runbook.md`, `scripts/platform-start.mjs` | UAT covers explicit env, migration, start, login, seed, `/app`, separate HTTPS origins, and header-only finalization. It uses placeholders and forbids secrets/private data in docs. |
...
| --- | --- | --- |
| Google/OIDC configuration | Generic OIDC config supports Google endpoints through env, verifies state/nonce, RS256 JWKS, issuer, audience, expiry, subject, nonce, and verified email. Verified normalized email may match a persisted pending workspace-membership approval for controlled onboarding. | For hosted alpha, configure Google redirect URI to the hosted HTTPS Platform callback. A valid identity without pending Platform approval receives safe approval-required denial; active membership and app entitlement remain separate checks. Do not use email/domain environment variables as admission authority. |
