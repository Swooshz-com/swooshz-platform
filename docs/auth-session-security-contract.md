...
- Origin/Referer checks for state-changing admin routes: member add, pending approval revoke, role change, membership disable, membership reactivation, and KQAG entitlement status mutation routes call the shared request-security helper before mutation.
...
- Audit events for workspace/app-access admin actions: membership add, membership approval create/revoke/accept, role change, membership disable, membership reactivation, KQAG entitlement enable, and KQAG entitlement disable append privacy-minimized audit events through the workspace admin service.
