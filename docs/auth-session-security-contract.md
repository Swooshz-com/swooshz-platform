...
- Provider-backed user requirement: sign-in resolves a provider-backed Platform identity only when the account is currently authorized by an active workspace membership in an active workspace or by a matching pending workspace membership approval. Existing active provider-backed users can be added immediately by admin and then sign in. Unknown teammates can only create a Platform user/session after real provider-backed sign-in accepts a pending approval. Email/domain environment variables are unsupported and are not admission authority.
- Pending workspace approval activation: admin can create a pending workspace membership approval by normalized email and one of the current roles (admin, operator, or viewer) before sign-in. The reviewed seed CLI can create a pending `admin` approval only for first-admin bootstrap while the workspace has zero memberships. The approval does not create fake provider identities, does not bypass OAuth, and does not grant app/admin access until `GET /api/platform/auth/callback` completes real OIDC verification for the matching normalized email, creates the active membership, marks the approval accepted, and writes audit metadata.
...
- Membership removal session revocation: admin membership removal revokes active platform sessions for the removed user in the same workspace-admin transaction as membership removal and audit append. Already-revoked sessions keep their original revoked timestamp, and sessions for the acting admin or unrelated users are not revoked.
...
- Workspace-wide active-session viewer: deferred. There is no admin UI for all active sessions.
...
| Audit events for workspace/app-access admin actions | Implemented partial | `src/platform/workspace-admin-service.ts`; `tests/workspace-admin-service.test.mjs`; `tests/auth-platform-identity-resolver.test.mjs`; `docs/internal-alpha-platform-contract.md` | Admin audit browsing must show admin actions and membership approval create/revoke/accept with privacy-minimized metadata. | Audit export/filtering/retention workflows. |
| Workspace-wide active-session viewer | Deferred | `docs/internal-alpha-platform-contract.md`; this contract | Not required for narrow internal alpha if manual/operator incident process is approved. | Build admin session viewer with privacy limits. |
