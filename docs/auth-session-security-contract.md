...
- Provider-backed user requirement: sign-in resolves a provider-backed Platform identity. Existing active provider-backed users can be added immediately by owner/admin, and unknown teammates can only gain workspace access after real provider-backed sign-in accepts a pending approval.
- Pending workspace approval activation: owner/admin can create a pending workspace membership approval by normalized email and non-owner role before sign-in. The approval does not create fake provider identities, does not bypass OAuth, and does not grant app/admin access until `GET /api/platform/auth/callback` completes real OIDC verification for the matching normalized email, creates the active membership, marks the approval accepted, and writes audit metadata.
...
- Membership removal does not block Google auth: removing a workspace member does not delete the platform user or provider identity. It revokes that user's active platform sessions, revokes workspace, admin, and app access through current membership checks, and clears stale selected workspace state when session context is reloaded. Future sign-in can create a new session, but access is still limited to current active memberships.
...
| Provider-backed user requirement | Implemented | `src/auth/platform-identity-resolver.ts`; `src/platform/workspace-admin-service.ts`; `tests/auth-callback-service.test.mjs`; `tests/workspace-admin-service.test.mjs` | Allow workspace membership activation only for active provider-backed users or pending-approved emails after real OIDC sign-in. | Controlled account-linking workflow if product scope expands. |
...
| Membership removal session revocation | Implemented | `src/platform/workspace-admin-service.ts`; `src/platform/repositories.ts`; `tests/workspace-admin-service.test.mjs` | Removing a workspace member must revoke that user's active platform sessions without deleting the user, provider identity, or other workspace memberships. Future sign-in remains provider-backed and current-membership checked. | Add workspace-wide active-session review only after a separate privacy/product review. |
