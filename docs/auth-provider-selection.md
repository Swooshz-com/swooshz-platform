...
Provider claims are inputs to the platform identity boundary. They are not the workspace model, not app authorization, and not the KQAG/SAQG access model. Apps should receive only platform-scoped context through approved platform contracts.
...
For internal SAQG/KQAG UAT, Google OIDC is the first practical smoke target because it can exercise the existing real OIDC login, callback, platform session, internal access seed, and browser shell flow without adding a provider SDK.
