...
- PostgreSQL provider and backup/restore owner: approve the recommended Neon target or a replacement provider, backup cadence, restore owner, and restore test process. Recommended non-secret target: project `swooshz-platform`, region `Singapore / aws-ap-southeast-1`, database `swooshz_platform`, current transitional legacy authority `platform_app` unchanged until a separately authorised live transition, future dedicated migrator `platform_migrator` (not yet created), non-owning restricted runtime `platform_runtime`, pooled `DATABASE_URL` for runtime.
- Dedicated migrator/legacy-owner alignment: decide who approves the #128 ownership-transfer Design Lock and which of the two locked transfer paths the reviewed live execution uses: authorised provider/role-lifecycle authority, or bounded temporary SET-capable membership from the executing current owner with immediate revocation and complete read-back. The repository contract rejects a bare temporary `CREATEDB` grant to the target role as sufficient transfer authority (the target never receives temporary `CREATEDB`), admits exactly one protected bootstrap-superuser-granted automatic creator edge for `platform_migrator` per `DL-128-REPO-002` during creation/credential admission/ownership transfer only, requires credential/login validation before ownership transfer, and preserves provider-managed `public` ownership with only the exact migration `CREATE`/`USAGE` authority. Per `DL-128-REPO-003`, the provider/bootstrap authority then revokes the automatic edge after successful transfer/read-back; the final accepted migrator membership posture is zero migrator membership, and `platform_app` must be proven unable to grant itself membership or `SET ROLE platform_migrator` after finalisation.
