...
- `platform_migrator` is the intended future dedicated operator/migration role: `LOGIN`, `NOSUPERUSER`, `NOCREATEDB`. No temporary `CREATEDB` on `platform_migrator`. `NOCREATEROLE`, `NOREPLICATION`, `NOBYPASSRLS`, never used by the long-running Coolify application, and connected only through the future private projection `NEONDB_SWOOSHZ_PLATFORM_MIGRATOR_URL` plus the process-local operator alias `DATABASE_OPERATOR_URL`. The automatic bootstrap edge (`granted_role=platform_migrator`, `member=platform_app`, bootstrap-superuser grantor, `admin_option=true`, `inherit_option=false`, `set_option=false`) is rejected if any field, direction, grantor or multiplicity differs, and it is not removable by the creator through an ordinary `REVOKE`. The automatic bootstrap edge may exist only during creation, credential admission and ownership transfer. A supplemental grant `granted_role=platform_migrator`, `member=platform_app`, grantor `platform_app`, `SET=true`, `INHERIT=false` may exist only during the bounded transfer window and must be revoked by the same grantor before final acceptance. After the successful transfer and complete read-back, the provider/bootstrap authority revokes the automatic edge after successful transfer/read-back; final accepted `platform_migrator` membership posture is zero edges across granted-role, member and grantor positions, and `platform_app` cannot then grant itself membership or `SET ROLE platform_migrator`.
- `platform_runtime` is unchanged: non-owning, zero membership edges, no migration or default-ACL authority, and restricted to the accepted exact 39-record direct grant contract. It remains `NOLOGIN` until separately authorised under #122.
...
Creator-edge finality per `DL-128-REPO-003` (supersedes `DL-128-REPO-002` only for creator-edge finality): `SET=false` and `INHERIT=false` do not create a security boundary while `ADMIN=true` remains, because `platform_app` can grant itself a fresh `SET=true` or `INHERIT=true` edge and reacquire migrator authority. The automatic edge protects against accidents only: it lets the creator revoke its own accidental supplemental grants while it cannot remove the bootstrap grant itself. Provider revocation is mandatory before final acceptance: the provider/bootstrap authority revokes the automatic edge after successful transfer/read-back. Final migrator membership is zero.

Ownership-transfer correction: a bare temporary `CREATEDB` grant to `platform_migrator` followed by `ALTER DATABASE ... OWNER TO platform_migrator` from an unrelated current role is rejected. That does not prove that the executing role can assume the new owner. The target role never receives temporary `CREATEDB`; the executing current owner requires `CREATEDB` and the ability to `SET ROLE` to the new owner. A future live transfer may use only one of these paths:
...
- Path B - bounded temporary SET ROLE-capable membership. The executing current owner is proved to own the object being transferred, holds the exact required `CREATEDB` and `SET ROLE` authority, creates the target role as `NOLOGIN` with final non-administrative attributes, admits the exact protected bootstrap creator edge, proves credential/login admission (private password, `LOGIN` enablement, private projection installation and a validated direct read-only migrator connection) before any ownership transfer, performs only the authorised ownership transfers in one rollback-capable transaction, revokes only the supplemental `SET=true` edge by its own grantor with immediate revocation of the bounded window, proves the remaining automatic `ADMIN=true` edge still permits the latent self-escalation (`platform_app` can grant itself a fresh `SET=true` edge and `SET ROLE platform_migrator`) and revokes only that self-granted edge, and then the provider/bootstrap authority revokes the automatic edge after the successful transfer and complete read-back. The final accepted membership posture is zero `platform_migrator` edges across granted-role, member and grantor positions, after which `platform_app` cannot grant itself membership and cannot `SET ROLE platform_migrator`. No permanent membership-derived migration authority is allowed in the final state: final migrator membership is zero.

`public` schema ownership: the accepted read-only live evidence establishes the known live starting fact that `public` is owned by the predefined `pg_database_owner` role. PostgreSQL defines `pg_database_owner` as having exactly one implicit member: the current database owner. A `public` schema owned by `pg_database_owner` is therefore governed by the current database owner; `pg_database_owner` is not an independent provider role, and the repository carries no stale application-controlled-public transfer branch. A later database-ownership transfer to `platform_migrator` while `public` remains owned by `pg_database_owner` therefore makes `platform_migrator` the implicit role exercising the associated `public` owner authority; the disposable rehearsal proves and reports that consequence and its complete reverse rollback and does not claim `public` stays independently provider-controlled. A separate explicitly labelled stable-provider-owner fixture exists only as a distinct possible future topology and is not evidence for the live one. A fresh read-only live preflight confirms the exact live owner before any later operation, and `platform_migrator` receives only the reviewed migration authority. Any inconclusive owner result blocks mutation. The final live Design Lock chooses among only facts mechanically proven by that fresh read-only live preflight; the repository contract performs no live ownership mutation.
