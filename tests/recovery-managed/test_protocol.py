...
class ProtocolAndIdentityTests(unittest.TestCase):
    ...
    def test_exact_wire_arrays_and_frame_hash(self):
        ...
        profiles = load_json(ROOT / "tests/recovery-managed/fixtures/wire-kats.json")["profiles"]
        transition = profiles[0]["store_text"].encode()
        evidence = profiles[1]["store_text"].encode()
        wire = BACKEND.StoreWire.from_bytes("restore-ledger-transition-data.v2", transition)
        evidence_wire = BACKEND.StoreWire.from_bytes("restore-begin-evidence.v2", evidence)
        restore_begin = BACKEND.build_restore_begin(raw, b"\x33" * 32, wire, evidence_wire, tagged)
