...
Runtime dependency composition now lives under `src/runtime`. It assembles the approved Node adapter dependency object from an injected Drizzle database object, runtime config, a CSRF hash secret config, a deterministic `now` factory, and an injected CSRF token id factory. When auth dependencies are supplied, it also reads the existing auth config contract, uses the Drizzle auth state store, secure state/nonce factories, and an `AUTH_STATE_HASH_SECRET`-backed HMAC reference factory. This composition does not create a DB connection, run migrations, listen on a port, implement provider networking, or call KQAG.

Explicit Node bootstrap wiring also lives under `src/runtime`. It reads runtime and secret config from injected environment values, creates the DB client through the existing database boundary only during `start()`, composes dependencies, starts the server only through explicit `start()`, and closes the server plus owned DB client on `stop()`. When an OIDC adapter is injected, bootstrap can include auth start/callback dependencies and requires `AUTH_STATE_HASH_SECRET`; creating the bootstrap object and calling `start()` do not call provider networking. It does not run migrations or create KQAG flows.
