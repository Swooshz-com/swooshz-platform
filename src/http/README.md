...
The Node server factory does not listen on import and does not create live repositories. Production runtime config requires secure cookies, a valid public base URL, and explicit allowed origins. Auth runtime composition requires `AUTH_STATE_HASH_SECRET` only when an injected OIDC adapter enables auth routes. The injected OIDC adapter boundary remains the only provider seam; real provider networking, provider SDKs, token verification implementations, frontend login UI, KQAG launch adapters, app launch tokens, billing, deployment, live DB migration execution, and CLI auto-start remain out of scope.
